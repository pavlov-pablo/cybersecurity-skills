...
Build, audit, and maintain SIEM detection content — the rules that fire alerts. Distinct from `incident-triage` (responds when alerts fire) and from a future `soc-operations` skill (runs the SOC that triages alerts). This skill is the engineering layer: log coverage, rule authoring, tuning, and detection-as-code workflows.

Cross-references: `incident-triage` for what happens after the alert, `breach-patterns` (coming in this release) for detection ideas pulled from public breach disclosures.
...
- Building a SOC team or alert escalation criteria (future `soc-operations`)
- Active threat hunting (future `threat-hunting`)
...
| Novel / never-seen-before | Threat hunting (see future `threat-hunting`) | Hypothesis-driven SIEM search |
